...
3. **Run API discovery** from kubernetes-health skill
   ```bash
   uv run ${CLAUDE_PLUGIN_ROOT}/skills/kubernetes-health/scripts/discover_apis.py --pretty
   ```
...
8. **Aggregate results**:
   ```bash
   uv run ${CLAUDE_PLUGIN_ROOT}/skills/kubernetes-health/scripts/aggregate_report.py --output summary
   ```
