...
Usage:
    # In ${CLAUDE_PLUGIN_ROOT}/hooks/hooks.json:
    {
      "hooks": {
        "Notification": [{
          "hooks": [{"type": "command", "command": "uv run ${CLAUDE_PLUGIN_ROOT}/hooks/ssh_osc777_notification.py"}]
        }],
        "Stop": [{
          "hooks": [{"type": "command", "command": "uv run ${CLAUDE_PLUGIN_ROOT}/hooks/ssh_osc777_notification.py --emoji ✅"}]
        }]
      }
    }
...
Testing:
    ...
    echo '{"hook_event_name":"Notification","message":"Test","session_id":"test"}' | \\
      uv run ${CLAUDE_PLUGIN_ROOT}/hooks/ssh_osc777_notification.py --dry-run

    # Test different terminal types
    uv run ${CLAUDE_PLUGIN_ROOT}/hooks/ssh_osc777_notification.py --force-terminal iterm2 < test.json
"""
