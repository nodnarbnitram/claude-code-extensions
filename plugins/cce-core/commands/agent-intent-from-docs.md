...
The meta-agent should write the agent file to `${CLAUDE_PLUGIN_ROOT}/agents/specialized/<appropriate-category>/` with a descriptive filename that reflects the intent.
...
Each meta-agent should write its agent file to `${CLAUDE_PLUGIN_ROOT}/agents/specialized/<appropriate-category>/` with a descriptive filename that identifies the specific vertical.
