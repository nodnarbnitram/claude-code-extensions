...
The meta-agent should write the agent file to `${CLAUDE_PLUGIN_ROOT}/agents/specialized/<appropriate-category>/` with a descriptive filename.
...
Each meta-agent should write its agent file to `${CLAUDE_PLUGIN_ROOT}/agents/specialized/<appropriate-category>/` with a descriptive filename that identifies the specific vertical (e.g., `cloudflare-workers-expert.md`, `cloudflare-d1-expert.md`).
